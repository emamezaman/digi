@extends('layouts.admin') @section('title','ویرایش محصول') @section('header')
...
<div class="panel panel-primary">
    <div class="panel-heading">
        ویرایش - <span style="color: red">{{$product->title}}</span>
        <a data-toggle="tooltip" data-placement="top" title="لیست محصولات" href="{{route('product.index')}}" class="pull-left btn btn-info"><i class="icon-list"> </i>  </a>
         <a data-toggle="tooltip" data-placement="top" title="محصول جدید" href="{{route('product.create')}}" class="pull-left btn btn-success"><i class="icon-plus"> </i>  </a>
          <a data-toggle="tooltip" data-placement="top" title="حذف" href="{{route('product.destroy',$product->id)}}" class="pull-left btn btn-danger"><i class="icon-trash"> </i>  </a>
        <i class="clearfix"></i>
    </div>

    <div class="panel-body">
        {!!Form::model($product,['route'=>['product.update',$product->id],'files'=>'true','class'=>' form-horizontal'])!!} {{-- fild title fa --}}
      {{method_field('PATCH')}}
        ...
        <div class="form-group">
            {!!Form::label('cat_id','انتخاب سر دسته ',['class'=>'control-label col-md-2'])!!}
            <div class="col-md-9 ">
                {!!Form::select('cat[]',$cat_list,$product->categories,['class'=>'selectpicker','required','data-live-search'=>'true','multiple'=>'multiple'])!!} @if($errors->has('cat'))
        ...
        <div class="form-group  ">
            ...
            <div class="col-md-9 ">
                {!!Form::checkbox('product_status','1',$product->product_status ,['class'=>'vertical-middle'])!!}
        ...
        <div class="form-group  ">
            ...
            <div class="col-md-9 ">
                {!!Form::checkbox('show_product','1',$product->show_product,['class'=>'vertical-middle'])!!}
        ...
        <div class="form-group  ">
            ...
            <div class="col-md-9 ">
                {!!Form::checkbox('special','1',$product->special ,['class'=>'vertical-middle'])!!}
        ...
        {{-- ============================== --}}
          <div class=" ">
            <label class="control-label ">انتخاب رنگ</label><br>
            <div class=" ">
    ...
    <div id="color{{$color->code}}"  style="margin:8px 0;">
    
    <input type="text" value="{{$color->title}}" class=" color_input_name my_input " name="color_name[{{$color->id}}]">
    <input type="text" value="{{$color->code}}" class="jscolor color_input_code my_input" name="color_code[{{$color->id}}]">
    
    </div>
    @endforeach
    </div>
        <div id="color_box" class="form-group">
        ...
        <div class="form-group  ">
            ...
            <div class="col-md-8  ">
          <input type="text" id="taglist"  class="form-control text-left"   name="taglist"  >
          <input type="hidden" id="keyword" value="{{$product->keyword}}"  class="form-control"  name="keyword"  >
          
        <div   id="tag_box">
        @php
        $tag_list = explode(',',$product->keyword);
        @endphp
        @foreach ($tag_list as $key => $value) 
        @if(!empty($value))
    <div onclick="del_tag('t{{$key}}','{{$value}}')" id="t{{$key}}" class="tag_list">
        <span class="icon-remove"></span>{{$value}}</div>
        @endif
        @endforeach
        ...
        <div class="col-md-1">
        <span data-toggle="tooltip" title="افزودن برچسب" data-placement="top" onclick="tag_list()" class="btn btn-success">افزودن</span>
        </div>
        </div>
...
<div id="ff"></div>
        <div class="form-group">
            <div class="col-md-12">
                {!!Form::submit('ویرایش شو',['class'=>'btn btn-warning','title'=>'ویرایش نمودن','data-toggle'=>"tooltip", 'data-placement'=>'top'])!!}
            </div>
        </div>
        {!!Form::close()!!}
    </div>
